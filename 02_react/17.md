...
**COLOCAR UMA IMAGEM QUE PRESENTE ISSO**

**Linhas 26 a 30:** Adicionamos o hook useEffect que **será executado sempre que houver uma mudança no estado usuario**. Havendo uma mudança, **executamos a condicional que verifica se o ID do usuario não está mais com o seu valor inicial, ou seja, se o campo ID não está vazio. Se o ID não estiver vazio (ou seja, o usuário foi cadastrado com sucesso),** o código chama a constante navigate para redirecionar o usuário para a rota de Login.

**Linhas 32 a 34:** Como tínhamos visto antes, essa função serve para enviarmos o usuário para o Login, após o cadastro bem sucedido ou caso o botão cancelar seja pressionado.

**Linhas 36 a 38:** Essa função é usada para atualizar o estado de **confirmaSenha** com o valor inserido pelo usuário no campo de confirmação de senha. Sempre que o usuário digita algo nesse campo, essa função é chamada e atualiza o estado. 

**Linhas 40 a 45:** Esta função é usada para atualizar o estado usuario de acordo com os valores fornecidos no inputs do formulário. Quando um elemento de input é alterado, esta função cria um novo objeto **usuario** com as propriedades existentes e a propriedade associada ao elemento que está sendo alterado é atualizada com o novo valor.

**Linha 40:** Iniciamos a estrutura da função, indicando que aceita um argumento denominado **e** do tipo **ChangeEvent**. Isso indica que a função é projetada para lidar com eventos de mudança em elementos **input**. Ou seja, ela será disparada sempre que um input tiver seu conteúdo alterado.

**Linha 41:** Chamamos a função setUsuario para atualizar o objeto usuario. Portanto dentro de seus parênteses abrimos as chaves ({ }) - indicando que vamos modificar o objeto.

**Linha 42:** Aqui recorremos o uso do **Operador de Espalhamento (Spread Operator) caracterizado pelo uso de 3 pontos antes do objeto usuario**. Dentro da função, aqui estamos criando um novo objeto usuario com todas as propriedades existentes e, ao mesmo tempo, fazer modificações em propriedades específicas. Ele é útil para criar novos objetos baseados em objetos existentes com algumas alterações.

**Linha 43:** Nesta linha criamos uma propriedade dinâmica no objeto resultante. O nome da propriedade é definido como o valor de **e.target.name**, que corresponde ao atributo **name** do elemento **input**. O valor da propriedade é definido como o valor de **e.target.value**, que é o valor atual do input que disparou o evento de mudança.

**Linhas 47 a 69:** A função **cadastrarNovoUsuario** é responsável por lidar com o processo de cadastro de um novo usuário. Ela valida as senhas, executa o cadastro (ou exibe erros) com base nas condições especificadas e fornece feedback ao usuário por meio de alertas. Também controla o estado **isLoading** para indicar o progresso do processo de cadastro.

**Linhas 47 e 48:** Em sua assinatura, indicamos que a função irá lidar com o evento de envio de um formulário. Ela impede o comportamento padrão de recarregar a página ao clicar no botão de envio (graças ao e.preventDefault()). Isso irá permitir que a função de cadastrarNovoUsuario execute a lógica necessária sem recarregar a página.

**Linhas 50 e 51:** A condicional **if (confirmaSenha === usuario.senha && usuario.senha.length >= 8)** verifica se a senha inserida pelo usuário e a confirmação de senha são iguais e se a senha tem pelo menos 8 caracteres. Se essas condições forem atendidas, o código dentro desse bloco será executado. Já a função **setIsLoading(true);** define o estado **isLoading** como verdadeiro para indicar que o processo de cadastro está em andamento.

**Linha 53 e 58:** Adicionamos a estrutura de tratativa de erro Try/Catch para podemos lidar com qualquer erro que possa acontecer ao fazermos a requisição ao Back End.

**Linha 54:** Dentro do bloco **TRY**, invocamos a service **cadastrarUsuario**. Seguindo sua estrutura, passamos a URL da rota de cadastro definida no Back End, os dados a serem cadastrados através do argumento **usuario** e a função **setUsuario** que é usada para atualizar o estado do usuário após a conclusão do processo. A palavra-chave **await** indica que essa operação é assíncrona e aguarda a conclusão antes de continuar.

**Linha 55:** Se o processo for bem-sucedido, esta linha exibe um alerta informando que o usuário foi cadastrado com sucesso.

**Linha 56 e 60:** Independentemente do sucesso ou falha do login, isso define isLoading como false, indicando que o processo de carregamento foi concluído.

**Linha 58:** Caso houve algum tipo de erro no processo dentro do bloco **TRY**, a estrutura **CATCH** captura o erro, prevenindo um exceção que possa travar ou comprometer o funcionamento da aplicação, e além disso, exibe um alerta, informando que ocorreu algum erro no processo.

**Linhas 63 e 68:** Esse bloco corresponde a estrutura **ELSE**, que será executado caso: As senhas digitadas nos campos de senha e confirmação de senha não sejam iguais OU a senha digitada tenha menos de 8 caracteres. Se qualquer uma dessas ações acontecerem, disparamos um alerta ao usuário, apagamos os campos relacionados a senha e finalizamos o processo de carregamento.
...
**Linhas 142 a 147:** Através de uma expressão ternária, utilizamos a propriedade **isLoading** para definir se o Componente loader **RotatingLines** será exibido, indicando que existe um processo que está em carregamento - que no caso se refere a chamada a API para fazer o Cadastro. Se **isLoading** tiver o valor false, mostramos o texto Cadastrar. Toda essa lógica é aplicada dentro da tag Button do HTML.
